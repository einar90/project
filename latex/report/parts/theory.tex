...
\section{Theory} % (fold)
\label{sec:theory}
This section contains a thorough review of the equations discussed in Section~\ref{sec:introduction}. This includes a derivation of the expressions for the well index and equivalent wellbore radius originally derived by Peaceman \cite{Peaceman1978Interpretation}. The discretized form of the differential flow equation -- which is needed to calculate the equivalent wellbore radius -- is also derived.
...
The correct value for the equivalent wellblock radius $r_{eq}$ may be found by examining a numerical solution to the \emph{differential flow equation} on an $M\times M$ square grid like the one shown in Figure~\ref{fig:peaceman-grid}.
...
\begin{subequations}
    ...
    \end{equation}
\end{subequations}
where $O(\Delta x^2)$ and $O(\Delta x^2)$ are the error-terms. Substituting this and $A=\Delta x \Delta y$ into \eqref{eq:differential-flow} and taking $\Delta x = \Delta y$ gives us the discretized form of the differential flow equation
\begin{equation}
...
\end{equation}
Substitution of these conditions gives us an $(M+1)\times (M+1)$ equation system, which may be formulated as
\begin{equation}
...
\begin{figure}[htbp]
    ...
    \caption{Sketch of the equation system \eqref{eq:equation-system-matrix} \hl{generated by} equation \eqref{eq:differential-flow-discretzed-dimensionless} with $M=4$.}
