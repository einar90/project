...
One method for acquiring the correct value of the equivalent radius is by creating a semi-logarithmic plot of the pressure difference $(\Phi_{i,j}-\Phi_{0,0})$ against the radius from the producer $r/\Delta x=\sqrt{i^2+j^2}$ and extrapolate the regression line down to the horizontal line where $(\Phi_{i,j}-\Phi_{0,0})=0$ \cite{Peaceman1978Interpretation}. One such plot is shown in Figure~\ref{fig:peaceman77_pressure_vs_radius}. We then see that
\begin{equation}
    r_{eq} = 0.2 \Delta x
\end{equation}
This result is used by Peaceman for a new interpretation of the wellblock pressure: it is the steady-state flowing pressure at $r=r_{eq}=0.2 \Delta x$ \cite{Peaceman1978Interpretation}.

Values of $r_{eq}$ calculated using the regression method for various grid sizes are shown in the Results section of this report.

\begin{figure}[htbp]
    \centering
    \includegraphics[]{figures/plots/peaceman77-regression.pdf}
    \caption{Plot of numerical solution to equation~\eqref{eq:differential-flow-discretzed-dimensionless} plotted agains radius from producing well, along with the linear regression line. This plot is made from the solution on a $10\times 10$ grid.}
...
\end{equation}
which fits well with value of $r_{eq}$ acquired from inspecting intersection of the regression line \cite{Peaceman1978Interpretation}.
